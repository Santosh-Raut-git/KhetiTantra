...
import { LayoutDashboard, Users, Sprout, Wallet, Shield, ArrowLeft } from 'lucide-react-native';
import { theme } from '@/lib/theme';
import { useStore } from '@/lib/store';
import { useIsAdmin } from '@/lib/api/admin';
import { View, Text, ActivityIndicator, Platform, useWindowDimensions, Pressable, Image } from 'react-native';
import { SafeAreaView, useSafeAreaInsets } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';

// ---------------------------------------------------------------------------
// Admin Sidebar (web only, wide viewport)
// ---------------------------------------------------------------------------
const ADMIN_NAV_ITEMS = [
  ...
  { name: 'transactions', title: 'Transactions', Icon: Wallet, href: '/admin/transactions' },
] as const;
...
function AdminSidebar() {
  ...
  const getIsActive = (name: string) => {
    if (name === 'index') return pathname === '/' || pathname === '' || pathname === '/admin';
    ...
    >
      ...
      >
        ...
        >
          ...
          <Text style={{ fontFamily: 'Inter-Medium', fontSize: 13, color: '#6D4C41', marginLeft: 6 }}>
            Back to App
          </Text>
        </Pressable>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          />
          <View>
            <Text style={{ fontFamily: 'Inter-Bold', fontSize: 18, color: theme.colors.text }}>
              Super Admin
            </Text>
  ...
  if (!isAdmin) {
    return (
      <SafeAreaView className="flex-1 bg-sand justify-center items-center px-8">
        ...
        <Text className="text-soil text-2xl font-bold mt-6 text-center" style={{ fontFamily: 'Inter-Bold' }}>
          Access Denied
        </Text>
        <Text className="text-soil-muted text-base text-center mt-3" style={{ fontFamily: 'Inter-Regular' }}>
