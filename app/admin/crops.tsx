...
import { useAdminCrops, useDeleteCropAdmin, AdminCrop } from '@/lib/api/admin';
...
export default function AdminCropsScreen() {
  ...
  const handleDeleteCrop = (crop: AdminCrop) => {
    Alert.alert(
      'Delete Crop',
  ...
  const getStatusBg = (status: string) => {
    switch (status) {
      case 'active': return 'bg-leaf/10';
      case 'harvested': return 'bg-harvest/10';
      case 'failed': return 'bg-clay/10';
      default: return 'bg-soil/5';
    }
  };
                  ...
                  <View className="flex-row items-start flex-1">
                    ...
                    <View className="flex-1">
                      <View className="flex-row items-center justify-between">
                        ...
                        <View className={`px-2.5 py-1 rounded-full ${getStatusBg(crop.status)}`}>
                          <Text className={`text-xs font-bold capitalize ${getStatusColor(crop.status)}`} style={{ fontFamily: 'Inter-Bold' }}>
                            {crop.status}
                          </Text>
                        </View>
                      </View>
