...
import { useCrop, useUpdateCrop } from '@/lib/api/crops';
...
type CropEditFormData = z.infer<typeof cropEditSchema>;
...
export default function EditCropScreen() {
  ...
  const { control, handleSubmit, formState: { errors }, reset, watch } = useForm<CropEditFormData>({
    resolver: zodResolver(cropEditSchema),
    defaultValues: {
      status: 'active',
    }
  });
  ...
  useEffect(() => {
    if (crop && !isReady) {
      reset({
        ...
        season: crop.season,
        area_acres: crop.area_acres ? crop.area_acres.toString() : '',
        sowing_date: crop.sowing_date,
        expected_harvest_date: crop.expected_harvest_date || '',
        status: crop.status,
        actual_harvest_date: crop.actual_harvest_date || '',
        yield_quantity: crop.yield_quantity ? crop.yield_quantity.toString() : '',
      });
      setIsReady(true);
    }
  }, [crop, reset, isReady]);

  const onSubmit = (data: CropEditFormData) => {
    updateCrop.mutate({
      id,
  ...
  return (
    <SafeAreaView className="flex-1 bg-sand" edges={['top']}>
      <View className="flex-row items-center px-5 pt-4 pb-4 border-b border-soil/5">
        <Button 
          variant="secondary" 
          label="" 
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1">
        <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="p-5 pb-12">
            ...
            {watchStatus === 'harvested' && (
              <View className="mt-2 p-4 bg-harvest/10 rounded-2xl border border-harvest/30">
                ...
                <Controller
                  ...
                  render={({ field: { onChange, value } }) => (
                    <Input
                      label={`Yield Quantity (${crop?.yield_unit || 'quintal'})`}
                      value={value}
                      onChangeText={onChange}
                      keyboardType="numeric"
          ...
          <Button 
            label={updateCrop.isPending ? 'Saving...' : 'Save Changes'} 
