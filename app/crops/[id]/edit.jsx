  ...
  .refine(
    (data) => {
      if (data.expected_harvest_date && data.sowing_date) {
        return (
          new Date(data.expected_harvest_date) >= new Date(data.sowing_date)
        );
      }
      return true;
    },
    ...
    updateCrop.mutate(
      {
        ...
        actual_harvest_date:
          data.status === 'harvested'
            ? data.actual_harvest_date || undefined
            : undefined,
        yield_quantity:
          data.status === 'harvested' && data.yield_quantity
            ? parseFloat(data.yield_quantity)
            : undefined,
      },
      {
        onSuccess: () => {
          ...
          <View className="bg-surface p-5 rounded-3xl border border-soil/5 mb-6">
            ...
            {watchStatus === 'harvested' && (
              <View className="mt-2 p-4 bg-harvest/10 rounded-2xl border border-harvest/30">
                <Text
                  className="text-soil text-base font-bold mb-3"
                  style={{ fontFamily: 'Inter-Bold' }}
                >
                  Harvest Details
                </Text>
