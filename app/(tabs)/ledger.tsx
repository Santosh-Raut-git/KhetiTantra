...
import { useTransactions, Transaction } from '@/lib/api/transactions';
import { Button } from '@/components/ui/Button';
import { Select } from '@/components/ui/Select';
import { EmptyState } from '@/components/ui/EmptyState';
import { Plus, Receipt, ChevronRight, Filter } from 'lucide-react-native';
import { useCrops } from '@/lib/api/crops';
import Animated, { FadeInUp, useSharedValue, useAnimatedStyle, withSpring } from 'react-native-reanimated';

type TransactionWithCrop = Transaction & { crops?: { crop_name: string } };

type TransactionRowProps = {
  tx: TransactionWithCrop;
  index: number;
  onPress: () => void;
};

function TransactionRow({ tx, index, onPress }: TransactionRowProps) {
  ...
  return (
    ...
    >
      ...
      >
        <View className={`w-12 h-12 rounded-full items-center justify-center mr-3 ${tx.type === 'income' ? 'bg-leaf/10' : 'bg-clay/10'}`}>
          <Receipt size={22} color={tx.type === 'income' ? '#2E7D32' : '#D32F2F'} strokeWidth={2} />
        </View>
...
export default function LedgerScreen() {
  ...
  const { data: transactions, isLoading, error } = useTransactions(filterCrop || undefined);
  ...
  return (
    <SafeAreaView className="flex-1 bg-sand" edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="px-5 pb-8 pt-2 md:max-w-3xl md:w-full md:self-center md:py-6">
