...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setLandArea(profile.land_area_acres ? profile.land_area_acres.toString() : '');
    } else {
      fetchProfile();
    }
  }, [profile]);
  ...
  return (
    <SafeAreaView className="flex-1 bg-sand" edges={['top']}>
      <View className="flex-row items-center justify-between px-5 pt-4 pb-2 border-b border-soil/5 md:max-w-lg md:w-full md:self-center">
        ...
        {isAdmin && (
          ...
          >
            ...
            <Text className="text-leaf font-bold ml-2 text-sm" style={{ fontFamily: 'Inter-Bold' }}>
              Admin Panel
            </Text>
          </Pressable>
        )}
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="px-5 pb-8 pt-4 md:max-w-lg md:w-full md:self-center md:py-8">
        <Animated.View entering={ZoomIn.duration(500)}>
          <View className="items-center mb-6">
            ...
            {profile?.full_name ? (
              <Text className="text-soil text-lg font-bold mb-0.5" style={{ fontFamily: 'Inter-Bold' }}>
                {profile.full_name}
              </Text>
            ) : null}
            <Text className="text-soil-muted text-sm" style={{ fontFamily: 'Inter-Regular' }}>
              {session?.user?.email}
            </Text>
          </View>
        </Animated.View>
