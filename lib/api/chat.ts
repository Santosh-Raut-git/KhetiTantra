...
// ---------------------------------------------------------------------------
// Gemini REST API — works in React Native with no native dependencies
// ---------------------------------------------------------------------------
...
async function callGemini({
  ...
  // Build the full contents array: prior turns + current user message
  const contents = [
    ...history
...
export function useChatHistory() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!conversationId)
        return { conversationId: '', messages: [] as AiMessage[] };
...
export function useSendMessage() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // 1. Save user message first so it appears immediately after optimistic UI
      ...
      // 2. Fetch personalisation context (profile + active crops)
      const {
        data: { user },
      } = await supabase.auth.getUser();
      if (!user) throw new Error('Not authenticated');
      ...
      const systemPrompt = `You are the KhetiTantra AI Assistant, an expert agronomist for Indian farmers.
User Profile:
...
5. Keep responses concise but informative.`;
      ...
      if (aiInsertError) {
        console.warn('Failed to save AI response:', aiInsertError.message);
      }
    ...
    onError: () => {
      // Still refresh so the saved user message shows up
      queryClient.invalidateQueries({ queryKey: ['chat_history'] });
    },
  });
}
