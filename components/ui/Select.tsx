...
export function Select({ label, value, onValueChange, options, placeholder = 'Select...', error }: SelectProps) {
        ...
        >
          <View className="bg-surface rounded-t-3xl pb-8 max-h-[70%]">
            <View className="px-5 py-4 border-b border-soil/5">
              <Text className="text-soil text-lg font-bold" style={{ fontFamily: 'Inter-Bold' }}>
                {label || 'Select option'}
              </Text>
            </View>
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Text 
                    ...
                    style={{ fontFamily: item.value === value ? 'Inter-Bold' : 'Inter-Regular', fontSize: 16 }}
                  >
                    {item.label}
                  </Text>
                </Pressable>
